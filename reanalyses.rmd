...
These analyses are in support of a draft [Communications Arising](www.nature.com/nature/authors/gta/commsarising.html) manuscript that we plan to Nature in reply to Levin *et al*. The draft of the manuscript is available at https://github.com/caseywdunn/communication_arising.md . Feedback on the analyses and text is welcome. 
...
The KS tests as reproduced here are still significant, but the p values are much lower than those published by Levin *et al.*:
...
The R and matlab results are consistent with each other when analyzing `Figure_4d_data.txt`, indicating that my results don't differ from those of Levin *et al.* due to software differences and that the discrepency is due to the duplicated values.
...
Here I take a look at the matched values for each comparison.

```{r mathched}


ggplot(data=Dm, aes(x=phase,y=similarity, group=interaction(Species_A, Species_B))) + geom_point() + geom_line()
#ggplotly()

inverse_hourglass = D[ (D$Mid<D$Early) & (D$Mid < D$Late),]
inverse_hourglass
nrow(inverse_hourglass)

nrow(inverse_hourglass[inverse_hourglass$Species_B == "Ctenophores",])

table(c(as.character(inverse_hourglass$Species_A), as.character(inverse_hourglass$Species_B)))

```

So only `r nrow(inverse_hourglass)` of the `r nrow(D)` comparisons show an inverse hourglass patter. Of these, `r nrow(inverse_hourglass[inverse_hourglass$Species_B == "Ctenophores",])` are for pairs that include the ctenophore.


## Figure 4c data

These data are an upstream intermediate that the 4d data are dervived from. I explored these data a bit to better understand the broader analysis workflow.
